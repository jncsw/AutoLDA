...
class LDA_classifier(BaseEstimator, ClassifierMixin):
	...
	def __init__(self, max_df, min_df, topic_number, score_type, embedding_model):
	# def __init__(self):
		self.max_df = max_df
		self.min_df = min_df
	...
	def fit(self, train_data):
		...
		self.vectorizer = CountVectorizer(max_df=self.max_df, min_df=self.min_df, max_features=1000)
	...
	def score(self, train_data):
		...
		score.append(self.lda_model.perplexity(tmp))

		train_data_tmp = [text_tmp.split(' ') for text_tmp in train_data]
		vocab = np.array(self.vectorizer.get_feature_names())
		topic_word_distrib = np.array(self.lda_model.components_)
...
if (not load_model):
	...
	lda = LDA_classifier(max_df=0.9, min_df=0.05, topic_number=7, score_type=score_type, embedding_model=embedding_model)
	lda.fit(train_data)
	save_model_name = 'saved_models_for_testing/LDA_v0.pkl'
	if save_to_model:
...
vectorizer, lda_model = lda.get_model()
