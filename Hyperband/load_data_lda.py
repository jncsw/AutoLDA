...
def lemmatization(doc, nlp, allowed_postags=['NOUN', 'ADJ', 'VERB', 'ADV']): #'NOUN', 'ADJ', 'VERB', 'ADV'
	texts_out = []
	doc = nlp(doc)
...
def main():
	...
	for root, dirs, files in os.walk('../Transcripts'):
		for file in files:
			if file.endswith('.txt') and (file.split('.')[0] in filenames):
				...
				script = " ".join(script)
				script = lemmatization(script, nlp)
	...
	# save the train_data as pickle file
	print('\nsave train_data to pickle file ...')
	with open('train_data.pkl', 'wb') as fh:
   		pickle.dump(train_data, fh)

if __name__ == "__main__":
	main()

# the train_data in used 