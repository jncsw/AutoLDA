...
param_dist = {"max_df": list(np.linspace(0.4, 0.6, 2)),					
			  "min_df": list(np.linspace(0.1, 0.2, 2)),		 
			  "topic_number": list(np.linspace(4, 5, 2, dtype='int')),	
			  }
...
class LDA_classifier(BaseEstimator, ClassifierMixin):
	...
	def fit(self, train_data):
		...
		self.lda_model = LatentDirichletAllocation(n_components=self.topic_number, learning_method='online', random_state=100, max_iter=100)
